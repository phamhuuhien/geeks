...
public class SipHome extends SherlockFragmentActivity implements OnWarningChanged {
	...
	private String HOST = "173.198.254.66";

	private final String N_SIPSERVER = "n_sipserver";
	private final String STATUS = "status";
	private final String N_USERNAME = "n_username";
	private final String N_SIPPROXY = "n_sipproxy";
	private final String N_PASSWORD = "n_password";
	private final String N_SIPPORT = "n_sipport";
	private String n_sipserver, n_status, n_username, n_sipproxy, n_password, n_sipport;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		sipuser = getIntent().getStringExtra(MainActivity.SIPUSER);
		sippass = getIntent().getStringExtra(MainActivity.SIPPASS);
		SharedPreferences mPrefs = getSharedPreferences("LSprefs",0);
		//long accountId = Long.parseLong(mPrefs.getString("accountId", "-1"));
		//System.out.println("accountId="+accountId);
		//buildAccount();
		if(checkForUnlimitAccount()) {
			buildAccount(false);
		} else {
			...
			new Thread(new Runnable() {
				@Override
				public void run() {
					getUnlimitAccount(sipuser);
				}}).start();
		}
	}

	public void buildAccount(boolean isTrial) {
		if(isTrial) {
			List<SipProfile> tmp = SipProfile.getAllProfiles(this, false);
			if(tmp.size() <= 0) {
				SipProfile account = SipProfile.getProfileFromDbId(this, SipProfile.INVALID_ID, DBProvider.ACCOUNT_FULL_PROJECTION);
				Log.d(THIS_FILE, "begin of save ....");
				String accountUserName = sipuser;
				String accountPassword = sippass;
				account.display_name = accountUserName;

				account.acc_id = "<sip:" + accountUserName + "@"+HOST+">";

				String regUri = "sip:" + HOST + ":5000";
			...
			///if(tmp.size() == 1) {
			//	getContentResolver().delete(ContentUris.withAppendedId(SipProfile.ACCOUNT_ID_URI_BASE, tmp.get(0).id), null, null);
			//}
			if(tmp.size() <= 0) {
				SipProfile account = SipProfile.getProfileFromDbId(this, SipProfile.INVALID_ID, DBProvider.ACCOUNT_FULL_PROJECTION);
				Log.d(THIS_FILE, "begin of save ....");
				String accountUserName = n_username;
				String accountPassword = n_password;
				account.display_name = accountUserName;

				account.acc_id = "<sip:" + accountUserName + "@"+n_sipserver+">";

				String regUri = "sip:" + n_sipserver + ":" + n_sipport;
				account.reg_uri = regUri;
				String proxy_uri = "sip:" + n_sipproxy + ":" + n_sipport;
				account.proxies = new String[] { proxy_uri } ;
	...
	public boolean checkForUnlimitAccount() {
		SharedPreferences mPrefs = getSharedPreferences("LSprefs",0);
		n_sipserver = mPrefs.getString(N_SIPSERVER, null);
		n_status =  mPrefs.getString(STATUS, null);
		n_username = mPrefs.getString(N_USERNAME, null);
		n_sipproxy = mPrefs.getString(N_SIPPROXY, null);
		n_password = mPrefs.getString(N_PASSWORD, null);
		n_sipport = mPrefs.getString(N_SIPPORT, null);
		if(n_sipserver != null) return true;
		else return false;
	}

	public void getUnlimitAccount(final String user) {
		HttpClient client = new DefaultHttpClient();  
		HttpPost post = new HttpPost("http://173.198.254.66:8000/pbx/check_signup_status/"); 
		...
		try {
			...
			obj.put("imei", mTelephonyMgr.getDeviceId());
			...
			final JSONObject infor = new JSONObject(message);
			final String status = infor.getString(STATUS);
			final String description = infor.getString("description");
			mHandler.post(new Runnable() {
				@Override
				public void run() {
					progressDialog.dismiss();
				}});
			if(code == 0 && infor.getString(STATUS).equals("Done")) {
				SharedPreferences mPrefs = getSharedPreferences("LSprefs", 0);
				SharedPreferences.Editor editor = mPrefs.edit();

				editor.putString(N_SIPSERVER, infor.getString(N_SIPSERVER));
				editor.putString(STATUS, infor.getString(STATUS));
				editor.putString(N_SIPPROXY, infor.getString(N_SIPPROXY));
				editor.putString(N_SIPPORT, infor.getString(N_SIPPORT));
				editor.putString(N_PASSWORD, infor.getString(N_PASSWORD));
				editor.putString(N_USERNAME, infor.getString(N_USERNAME));
				editor.commit();
				n_sipserver = infor.getString(N_SIPSERVER);
				n_status =  infor.getString(STATUS);
				n_username = infor.getString(N_USERNAME);
				n_sipproxy = infor.getString(N_SIPPROXY);
				n_password = infor.getString(N_PASSWORD);
				n_sipport = infor.getString(N_SIPPORT);
				...
				mHandler.post(new Runnable() {
					...
					public void run() {
						if(status.equals("None")) {
							//CommonHelper.showWarningDialog(SipHome.this, "Trial account" , "Let signup to have unlimit account");
							...
							builder.setMessage("Signup to have unlimit account?").setPositiveButton("Yes", dialogClickListener)
							.setNegativeButton("No", dialogClickListener).show();
						} else {
							CommonHelper.showWarningDialog(SipHome.this, status , description);
						}
						buildAccount(true);
					}});
			}
	...
	public void requestUnlimit() {
		HttpClient client = new DefaultHttpClient();  
		HttpPost post = new HttpPost("http://173.198.254.66:8000/pbx/signup/"); 
		post.setHeader("Content-type", "application/json");
		...
		try {
			...
			mTelephonyMgr = (TelephonyManager) getSystemService(Context.TELEPHONY_SERVICE);
			SharedPreferences mPrefs = getSharedPreferences("LSprefs",0);
			String username = mPrefs.getString(MainActivity.SIPUSER, null);
			JSONObject obj = new JSONObject();
			...
			mHandler.post(new Runnable() {
				...
				public void run() {
					progressDialog.dismiss();
					if(code == 0) {
						Intent myIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://www.telecomgeeks.com/mobile/Product.aspx?ProductCode=MOBILE1-US"));
						startActivity(myIntent);
					} else {
						try {
							final JSONObject error = new JSONObject(message);
							CommonHelper.showWarningDialog(SipHome.this, "error:"+error.getString("description"));
						} catch (Exception e) {
